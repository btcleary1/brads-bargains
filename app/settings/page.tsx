...
function SettingsContent() {
  ...
  const [maxDaysToSell, setMaxDaysToSell] = useState<number | ''>(60);
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/me').then(r => {
      ...
      fetch('/api/prefs').then(r => r.ok ? r.json() : {}).then((p: any) => {
        ...
        if (p.maxDaysToSell != null) setMaxDaysToSell(p.maxDaysToSell);
        if (p.filterPrefs) {
  ...
  const handleEnablePush = async () => {
    ...
    try {
      ...
      // Unsubscribe from any existing subscription first (different VAPID key would cause "string did not match" error)
  ...
  const handleTestPush = async () => {
    ...
    try {
      ...
      else setPushMessage({ type: 'error', text: (d as any).error || `Error ${res.status}` });
    ...
    try {
      const res = await fetch('/api/prefs', {
        ...
        body: JSON.stringify({
          ...
          maxDaysToSell: maxDaysToSell === '' ? null : maxDaysToSell,
        }),
      });
      if (res.ok) {
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(160deg,#050814 0%,#0B1120 60%,#0f172a 100%)' }}>
      ...
      <div className="max-w-xl mx-auto px-4 py-6 pb-24 sm:pb-10">
        ...
        <div className="rounded-2xl p-5 mb-4" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <p className="text-xs mb-4" style={{ color: '#6B7280' }}>Pre-fill deal filters. Items with no sell-time data (N/A) always show through.</p>
          {dealFiltersMessage && (
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs shrink-0" style={{ color: '#9CA3AF' }}>Max days to sell:</span>
            <input
              type="number"
              min={1}
              max={365}
              value={maxDaysToSell}
              onChange={e => setMaxDaysToSell(e.target.value === '' ? '' : Math.max(1, Number(e.target.value)))}
              placeholder="60"
              className="w-20 px-3.5 py-2.5 rounded-xl text-sm text-white placeholder-gray-600 outline-none focus:ring-2 focus:ring-blue-500/50"
              style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}
            />
